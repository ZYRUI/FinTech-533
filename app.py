...
app.layout = html.Div([
    ...
    html.Div([
        html.A('Benchmark: '),
        dcc.Input(id='benchmark-id1', type='text', value="IVV"),
        html.A('Asset: '),
        dcc.Input(id='asset-id1', type='text', value="AAPL.O"),
        html.A('Date range: '),
        dcc.DatePickerRange(
            id='date-range1',
            max_date_allowed=datetime.today(),
    ...
    html.Button('QUERY Refinitiv', id='run-query1', n_clicks=0),
    html.H2('Raw Data from Refinitiv'),
    dash_table.DataTable(
        id="history-tbl1",
        page_action='none',
    ...
    # html.Button('Plot Time Range', id = 'run-query-plot', n_clicks = 0),
...
def query_refinitiv(n_clicks, benchmark_id, asset_id, start, end):
    assets = [benchmark_id, asset_id, start, end]
...
# )
@app.callback(
    Output("history-tbl1", "data"),
    Input("run-query1", "n_clicks"),
    [State('benchmark-id1', 'value'), State('asset-id1', 'value'), State('date-range1', 'start_date'), State('date-range1', 'end_date')],
    prevent_initial_call=True
)
def query_refinitiv(n_clicks, benchmark_id, asset_id, start, end):
    assets = [benchmark_id, asset_id, start, end]
...
@app.callback(
    Output("returns-tbl1", "data"),
    Input("history-tbl1", "data"),
    prevent_initial_call = True
)
def calculate_returns(history_tbl):
...
@app.callback(
    ...
    Input("returns-tbl1", "data"),
    [State('benchmark-id1', 'value'), State('asset-id1', 'value')],
    #State('sub-date-range', 'start_date'), State('sub-date-range', 'end_date')],
    prevent_initial_call = True
)
...
def print_out_alphabeta(returns,benchmark_id,asset_id):
